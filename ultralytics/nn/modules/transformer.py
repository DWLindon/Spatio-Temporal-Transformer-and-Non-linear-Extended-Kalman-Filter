...
class STDFM(nn.Module):
    """Spatio-Temporal Deformable Feature Module with temporal cache.

    Args:
        c1 (int): Input channels.
        c2 (int): Output channels.
        num_points (int): Deformable sampling points per temporal slot.
        temporal_len (int): Number of temporal slots (current + history).
        cache_in_train (bool): If True, keep/update cache during training.
    """
    ...
    def __init__(
        ...
        num_points: int = 4,
        temporal_len: int = 3,
        cache_in_train: bool = False,
    ):
        super().__init__()
        self.c2 = c2
        self.num_points = num_points
        self.temporal_len = max(1, int(temporal_len))
        self.cache_in_train = cache_in_train
        self.use_temporal_in_eval = True
        self.in_proj = Conv(c1, c2, k=1, s=1)
        self.offset = nn.Conv2d(
            c2, 2 * self.temporal_len * num_points, kernel_size=3, stride=1, padding=1
        )
        self.attn = nn.Conv2d(c2, self.temporal_len * num_points, kernel_size=3, stride=1, padding=1)
        self.out_proj = Conv(c2, c2, k=1, s=1)
        self._feature_cache = None  # list[deque[Tensor[C,H,W]]], one queue per batch item
        self._cache_hw = None
    ...
    def _base_grid(h: int, w: int, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
        ...
        yy, xx = torch.meshgrid(ys, xs, indexing="ij") if TORCH_1_11 else torch.meshgrid(ys, xs)
        return torch.stack((xx, yy), dim=-1)  # [H, W, 2]

    def reset_cache(self) -> None:
        """Clear temporal feature cache explicitly."""
        self._feature_cache = None
        self._cache_hw = None

    def set_temporal_enabled(self, enabled: bool) -> None:
        """Enable temporal cache use in eval/video inference, or force static single-frame behavior."""
        self.use_temporal_in_eval = bool(enabled)
        if not self.use_temporal_in_eval:
            self.reset_cache()

    def _ensure_cache(self, b: int, h: int, w: int) -> None:
        # Clear stale cache when shape/batch changes to avoid cross-scene contamination.
        if self._feature_cache is None or self._cache_hw != (b, h, w):
            self._feature_cache = [deque(maxlen=max(0, self.temporal_len - 1)) for _ in range(b)]
            self._cache_hw = (b, h, w)

    def _build_temporal_tensor(self, x: torch.Tensor) -> torch.Tensor:
        """Build temporal tensor [B, T, C, H, W], slot 0 is current frame."""
        b, c, h, w = x.shape
        self._ensure_cache(b, h, w)
        temporal = []
        for bi in range(b):
            slots = [x[bi]]
            hist = list(self._feature_cache[bi])  # newest -> oldest
            for hi in range(self.temporal_len - 1):
                if hi < len(hist):
                    slots.append(hist[hi])
                else:
                    # Zero fallback keeps tensor shape fixed when history is short.
                    slots.append(torch.zeros_like(x[bi]))
            temporal.append(torch.stack(slots, dim=0))
        return torch.stack(temporal, dim=0)  # [B, T, C, H, W]

    def _update_cache(self, x: torch.Tensor) -> None:
        b = x.shape[0]
        for bi in range(b):
            self._feature_cache[bi].appendleft(x[bi].detach())

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.in_proj(x)
        b, c, h, w = x.shape

        # Training on shuffled still images has no meaningful temporal continuity.
        # If we are not caching in training, fall back to single-frame behavior to avoid attention mass wasted on zeros.
        use_temporal = ((not self.training) and self.use_temporal_in_eval) or (self.training and self.cache_in_train)
        t_eff = self.temporal_len if use_temporal else 1

        temporal = self._build_temporal_tensor(x) if use_temporal else x.unsqueeze(1)  # [B, T_eff, C, H, W]

        offsets = (
            self.offset(x)
            .view(b, self.temporal_len, self.num_points, 2, h, w)
            .permute(0, 1, 2, 4, 5, 3)[:,:t_eff]
        )  # [B,T_eff,P,H,W,2]
        attn = self.attn(x).view(b, self.temporal_len * self.num_points, h, w)
        attn = F.softmax(attn, dim=1).view(b, self.temporal_len, self.num_points, h, w)[:, :t_eff]

        # Normalize pixel-scale offsets to grid coordinates [-1, 1]
        norm = torch.tensor([max(w - 1, 1), max(h - 1, 1)], device=x.device, dtype=x.dtype)
        offsets = 2.0 * offsets / norm

        base = self._base_grid(h, w, x.device, x.dtype).unsqueeze(0).unsqueeze(0)  # [1,1,H,W,2]
        sampled = torch.zeros_like(x)
        for ti in range(t_eff):
            x_t = temporal[:, ti]  # [B,C,H,W]
            for p in range(self.num_points):
                grid = (base + offsets[:, ti : ti + 1, p]).squeeze(1)  # [B,H,W,2]
                feat = F.grid_sample(x_t, grid, mode="bilinear", padding_mode="border", align_corners=True)
                sampled = sampled + feat * attn[:, ti, p : p + 1]

        if (not self.training) or self.cache_in_train:
            self._update_cache(x)

        return self.out_proj(sampled + x)
